...
class TurboQuantAttentionImpl(AttentionImpl["TurboQuantMetadata"]):
    ...
    def forward(
        ...
        # Spec-decode K+1 verify routing (backport of upstream PR #40914).
        # A captured first-chunk prefill does not read the prior KV cache.
        ...
        if (
            ...
            _b = N // _k1
            _offs = torch.arange(
                _k1, device=device, dtype=attn_metadata.seq_lens.dtype
            )
            _synth_seq_lens = (
                attn_metadata.seq_lens[:_b, None] - _k1 + 1 + _offs[None, :]
            ).reshape(-1)
            _synth_bt = attn_metadata.block_table[:_b].repeat_interleave(
                _k1, dim=0
            )
